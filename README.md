...
| `run.py` | 한 번 실행. GitHub Actions가 03:13(KST)에 깨워 07:30까지 기다린 뒤 돌린다(예약 실행이 매번 2시간 넘게 늦게 만들어져서). 05:43·08:13 예비 실행은 그날 이미 보냈으면 건너뛴다 |
...
```
python test_graph_fake.py    # 모델·네트워크를 가짜로 바꿔 그래프 경로 22개 시나리오
...
- **손으로 돌리지 말 시간**: 03:10~08:30 KST. 대기 중인 예약 실행과 겹치면 예비 실행이 대기열에서 밀려날 수 있다
  - 예외: `once_a_day`를 켠 외부 호출. 이미 보낸 날은 건너뛴다
...
- **실행이 멈춰도 독자에게 알린다**
  - 도중에 예외로 멈추면 디스코드에 `⚠️ {원인} 오늘 브리핑을 만들지 못했습니다.`를 보낸다
  - 원인은 예외 종류·API 오류 코드로만 고른다 (예: 크레딧 소진 → `모델 사용 크레딧이 떨어져`). 오류 메시지는 옮기지 않는다
  - 하루 한 번만 보낸다. 예비 실행은 브리핑을 다시 시도하되 안내를 또 보내지 않는다
  - 브리핑을 이미 보낸 뒤 멈췄거나 dry-run이면 보내지 않는다
  - 2026-09-17 크레딧 소진으로 예약 실행 2번이 멈췄을 때 채널에 아무것도 안 가서 생긴 규칙이다
- **결과를 바꾸는 건너뜀만 기록한다**
