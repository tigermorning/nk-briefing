...
최종 갱신 2026-09-15 (과제 대조·REPORT.md·숫자 왜곡 검수). 다른 에이전트가 이어받기 위한 백로그. 로컬 실행 기준, API 키는 파일에만 존재(채팅에 없음).
...
15. **강의 6~13강 북한판** (`graph.py`, `audience.yaml`, `run.py`, `scorecard.py`, `.github/workflows/daily.yml`) — 2026-09-14
    ...
    - 선별: 후보 40건 넘을 때만 예선(묶음당 8), 본선은 TARGET+3 예비까지 받고 코드로 소스당 3건·같은 사건 제거 (TARGET은 22번에서 5 → 4, 발행은 3장)
...
22. **디스코드 피드백 반영 — 카드 수·스케치 기사** (2026-09-14 저녁)
    ...
    - 스케치 기사: `audience.yaml` 버릴_것에 "날씨·계절·여가·거리 풍경… 물가·식량·통제의 새 변화가 없는 스케치 기사" 추가. 같은 21건 풀(원장 무시)로 재선별 2회 → 공원 기사 후보에도 안 오름
    - **해 봤다가 뺀 것 — 기준 번호 관문**: `Pick.criterion`(해당 기준 번호, 없으면 0 → 코드가 뺌). 새 버릴_것 줄 없이 관문만 두고 같은 풀 2회 → 공원 기사에 2회 모두 **기준 4(내부 상황)**를 댐. 모델이 스스로 고른 기사를 "해당 없음"이라 하지 않는다. 효과 0이라 제거. 이 관문은 병렬 세션(`nk-briefing-7c`)이 작업 중간 상태를 `48e5e6f`로 커밋·push해 한때 origin에 들어갔다
    - 반영 후 실데이터 dry-run: 속보 12 → 4 취재, 검수 5→5, 발행 3장(속보 2 + 심층 아시아프레스), 예비 2
    - 남은 관찰: 이 dry-run에서 event 라벨이 다시 분야 이름 쪽으로 흐름(`북한 내 생산 현장 불만 및 통제 상황 변화`). 서로 다른 사건이라 잘리진 않았지만 같은 모양이 겹치면 중복제거가 오판할 수 있다

23. **과제 페이지 대조 · REPORT.md · 숫자 왜곡 검수** (2026-09-15)
    - 과제 페이지(모두의연구소 136 「나만의 뉴스레터 에이전트」) 필수 5단계와 제출물 대조. 새로 만든 것: `REPORT.md`(과제 6항목 순서), `docs/discord-2026-09-15-1.png`(사용자 캡처, 리드+카드 1·2만)
    - 9/15 07:30 예약 실행: GitHub가 **09:49 KST에 시작**(약 2시간 20분 지연). 1차 `OPEN` 첫 실데이터, 4장 실발송, 원장 커밋 `6776a78`
    - 독립 검증 결과 약점: 선별 `reason`이 기록에 없음 · 검수가 헤드라인·요약만 봄(💡 미검수) · 9/14 실발행 6장(3~5 초과, 규칙 도입 전)
    - **실제 발행 카드에 왜곡을 심은 실험** (`exp/step12_exaggeration.py` → `.log`, gpt-4.1-mini, 3회씩): 이전 검수는 `100억 → 1,000억/1천억 달러` 6/6, `최대 5만명` 삭제 3/3, `약…추정` 삭제 3/3, 💡 속 없는 사실 3/3을 **통과**. 합계 18/36
    - 반영 (`graph.py`, 새 `grounding.py`):
      - `grounding.py`: 숫자를 값으로 파싱(한·일·영, 조/억/만/천, 여, 전각, 영어 단어)해 필드별로 대조. `ungrounded`(값 없음, 맨숫자 10%·약 20%·여/이상 1.3배 허용) · `dropped_bounds`(최대/이상/up to 빠짐·방향 뒤집힘) · `dropped_hedges`(약/추정/estimated 빠짐). 날짜·시각·전화·URL 번호는 제외
      - `judge()` = 코드 대조 + LLM(`SYS_CHECK`에 💡 포함, 자릿수·대상·한정어·새 사실 명시, `Verdict.claims`를 `ok` 앞에). 불합격이면 `redraft()` 1회(지적 사항 전달) 후 같은 판정, 또 불합격이면 제외 → 예비가 칸 채움. metrics `check: {first_fail, rewritten, dropped}`
      - `SYS_DRAFT`: 원문 값만·계산/환산 금지·한정어 유지. 선별 로그 `+` 줄에 `이유:` 추가, 심층 선택도 로그
    - 새 검수 결과: **33/36**. 못 잡는 것은 숫자 없는 단정 강화(`전망된다 → 확실하다`) 3/3 통과뿐
    - gpt-4.1로 판정만 바꿔 봄: 한정어 삭제 3/3 잡지만 추정·전망 1~2/3, **정상 실제 발행본 1/3 오탈락** → 모델 교체 안 함
    - 코드 리뷰(별도 리뷰어) 3회: 1차 HIGH 3(본문 연도·전화번호가 반올림 근거, 옆 절의 "가능성"을 추정으로, "최고사령관"·"이상기후"를 한정어로), 2차 HIGH 2("여전히"를 추정 `여`로, "5만 시민"을 날짜로 → 10배 통과), 3차 HIGH 1(`8천t급 → 8만t급` 통과). 전부 수정, 리뷰어 문장 전부 `test_grounding.py`(97건). 최종 APPROVE
...
25. **REPORT 실행 이력 보완·문체** (2026-09-15)
    - 5.3 표에 Actions 수동 dry-run 두 줄이 빠졌던 이유: 표를 `metrics.jsonl`로 만들었는데 `daily.yml`은 dry-run이면 기록 커밋을 건너뛰어 행이 없음 + 두 실행(12:41 `34925978081`, 12:49 `34926461959`)이 표 작성 뒤였음. **Actions dry-run은 run 로그에서 옮겨 적을 것**
    - 5.3에 코드 열 추가(이전 검수 / 검수 보강), 9/14 Actions dry-run 두 줄 실제 수치(후보 11, 선별 6, 6→6)로 채움, 9/16 07:30 예정 행(새 코드 첫 실발송) 추가. RFA 17회 중 2회, 후보 범위 실행 17회 11~21건으로 갱신. 구조도 verify 노드·부록 테스트 목록 갱신
    - 문체: 해라체 유지, 어미 다양화(질문형·명사형·~셈이다 등). 수정 전후 숫자·코드 표기·코드 블록 자동 대조로 사실 보존 확인
...
2. **(개선·검수) 카나리아** — 23번 뒤에도 유효: 실데이터 불합격 0건이라 판정기가 일하는 모습은 심은 왜곡으로만 확인. 숫자 없는 단정 강화(확실·확정·분명)도 여기서 다룰 것. — 실제 실행 11회(로컬 9 + Actions 2) 검수 전부 N→N, 불합격 0. 판정기가 일하는지 모른다. `exp/step9_llm_check.py`의 가짜 유형(배율 과장·숫자 자리 바꾸기)을 매 실행 1건 섞고 못 거르면 경보. 우선순위 1
