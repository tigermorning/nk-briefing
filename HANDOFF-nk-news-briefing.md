...
11. `store/metrics.jsonl` 매 실행 append — 게이트 결과·소스별 기여·skips·elapsed
...
2. **1차枠 운영 방식 결정 (사용자 판단 대기)** — 통일부는 원문이 없는 요약 전용 소스로 확정됐다. 품질바의 "속보枠=원문필수"와 충돌한다. 선택지: (a) 1차枠만 원문필수 면제하고 요약 그대로 싣기 (b) 요약 생성 없이 제목+링크 한 줄로만 쓰기 (c) `cn >= 300` 인 날만 1차枠 열기(30일 중 15일)
